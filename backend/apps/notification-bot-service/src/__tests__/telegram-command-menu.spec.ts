...
assert(
  NOTIFICATION_BOT_COMMANDS.every((command) => !command.command.includes('card')),
  'notification bot command menu must not expose manual card commands',
);
assert(
...
assert(!formatTelegramCommandHelp().includes('/pending_cards'), 'notification help must not include pending card usage');
assert(!formatTelegramCommandHelp().includes('/approve_card'), 'notification help must not include approve card usage');
