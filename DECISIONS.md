...
- **Why**: Source-of-truth requires that raw VPN config handling be treated as a real ingest/normalize/runtime-preparation flow, not as vague ‚Äúdecryption‚Äù or direct string pass-through.
- **Impact**:
...
- **Why**: Real device testing showed that the current pain is not only ‚Äúmissing protocols‚Äù, but also brittle handling of real-world variants inside the protocols we already claim to support. It is more honest and more useful to make supported formats robust first.
- **Impact**:
...
- **Impact**:
  ...
  - This avoids a misleading state where Prisma history says ìappliedî while SQL never actually ran.
...
- **Impact**:
  ...
  - Users can distinguish ìpaid and being preparedî from ìactiveî and ìexpiredî.
...
- **Why**: The product trial is not a paid offer and should not visually compete with a supplier-backed paid plan or an imported configís parsed metadata. Showing the historical `5 GB` backend fallback made the profile look like trial was a normal limited plan, which created hidden confusion.
- **Impact**:
