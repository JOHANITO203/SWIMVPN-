...
- **GATE DEVICE FAIT (S23+, concluant)** : banc adb-reverse + serveur local (manifest test code 14, apkUrl=HTTPS prod). Validé en vrai : prod HTML→zéro nag ; dialog Optional + changelog FR ; download 38 Mo ; SHA-256 PASS → packageinstaller lancé (log-prouvé) ; throttle <24h→pas de re-nag. Mergé dans release/1.0.13.
...
- **P0 #2 — Comparatif paiements** : `docs/payments/2026-06-23-payment-options-comparison.md`. Multi-rail (MoR / crypto API-first / Telegram / RU-CIS / XOF) + reco. ✅ **Vérif live FAITE** (§9) : Paddle = VPN catégorie restreinte (pas Paddle) ; **2Checkout/Verifone = MoR de tête** (VPN non interdit, 3,5/4,5 %) ; Cryptomus à éviter (pénalité AML FINTRAC 177 M CAD) ; Coinbase Commerce fermé hors US/SG (31/03/2026) ; CoinGate exclut la RU ; NOWPayments sert la RU (non-custodial, 0,5-1 %) ; BTCPay 0 % ; QIWI mort ; **XOF = blocage entité régionale** ; Telegram Stars ~32 % perte mobile + données identité-seule.
...
**À FAIRE (court)** : (a) câbler l'**endpoint de désinscription** avant tout broadcast email ; (b) traduire les emails E1–E4 en **RU**. *(vérif live paiements = faite)*
...
**P0 #1 — Bot stock intelligent : IMPLÉMENTÉ + VÉRIFIÉ + MERGÉ main (`da0cfe9`)** (design `docs/superpowers/specs/2026-06-23-stock-intelligence-design.md`). Livré : seuils par plan (remplace le `5` codé en dur) + vélocité de conso (`OrderAssignment.assigned_at`) + prévision « ≈ X j » + suggestion réappro + **alerte proactive `stock_forecast_alert`** (pass planifiée dans le healthcheck 30 min, dedup/jour) + overview `/stock` enrichi (vélocité/runway/réappro par plan). Lecture seule, additif ; **auto-pause ventes = GATE** non incluse.
- **Vérif verte** : `tsc --noEmit` OK · policy spec OK · admin-bot-formatter + telegram-command-menu OK · `nest build` inventory/admin/notification OK · le seul rouge `inventory.service.spec.ts` est **prouvé pré-existant** (identique sur la base via stash).
- ⚠️ **DÉPLOIEMENT PROD EN ATTENTE** : le backend ne s'auto-déploie PAS (seul le landing le fait). Rollout manuel requis sur l'hôte Dokploy (`scripts/ops/deploy.sh` / ordre documenté ; `up -d --build` ciblé inventory/admin/notification + `--no-deps` pour éviter la course-incident). SSH depuis cette machine = publickey refusé en non-interactif. **Pas de migration, pas d'env requis** (défauts) → déploiement sans risque DB, partira au prochain rollout backend.
