...
export const LandingPage = () => {
  ...
  const bgY1 = useTransform(smoothProgress, [0, 1], [0, isMobile ? -100 : -300]);
  const bgY2 = useTransform(smoothProgress, [0, 1], [0, isMobile ? 100 : 300]);
  const heroY = useTransform(smoothProgress, [0, 0.3], [0, isMobile ? -30 : -100]);
  const globeScale = useTransform(smoothProgress, [0, 0.3], [1, isMobile ? 0.95 : 0.85]);
  const heroRotate = useTransform(smoothProgress, [0, 0.3], [0, isMobile ? -2 : -5]);
  const statsY = useTransform(smoothProgress, [0.3, 0.6], [50, -50]);

  return (
    <div ref={containerRef} className="min-h-screen w-full app-bg overflow-x-hidden transition-colors duration-500 selection:bg-minimal-accent/30">
      ...
      <nav className="fixed top-0 left-0 w-full z-50 backdrop-blur-md border-b app-border transition-all duration-300">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-black text-xl tracking-tighter app-text-bold uppercase">Swim VPN</span>
          </div>
          <div className="hidden md:flex items-center gap-8 text-[10px] font-black uppercase tracking-widest app-text-secondary">
            <a href="#features" className="hover:app-text-bold transition-colors">Features</a>
            <a href="#how-it-works" className="hover:app-text-bold transition-colors">How it works</a>
            <a href="#download" className="hover:app-text-bold transition-colors">Download</a>
          </div>
        </div>
      </nav>
      ...
      <header className="relative pt-40 pb-20 px-6 max-w-7xl mx-auto text-center md:text-left flex flex-col md:flex-row items-center gap-16">
        <motion.div style={{ y: heroY, rotateZ: heroRotate }} className="flex-1 space-y-8 z-10">
          <motion.div 
          ...
          >
            ACCESS BLOCKED CONTENT.<br />NO DROPS. <span className="text-minimal-accent dark:text-dark-accent">NO SETUP.</span>
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            <a href="#pricing" className="w-full sm:w-auto px-10 py-6 app-btn-primary rounded-[2rem] shadow-2xl flex items-center justify-center gap-3 font-black uppercase tracking-widest text-xs hover:scale-105 transition-all">
              <Zap className="w-5 h-5" />
              GET INSTANT ACCESS
            </a>
            <a href="#download" className="w-full sm:w-auto px-10 py-6 app-surface border app-border rounded-[2rem] flex items-center justify-center gap-3 font-black uppercase tracking-widest text-xs hover:bg-minimal-accent/5 transition-all">
              Download APK
            </a>
          </motion.div>
        </motion.div>

        <motion.div style={{ scale: globeScale, y: isMobile ? 0 : heroY }} className="flex-1 relative w-full h-[500px] md:h-[600px] flex items-center justify-center">
          ...
          <div className="absolute top-10 right-10 z-20 hidden md:block animate-float">
             <div className="app-card p-6 border app-border rounded-2xl shadow-xl backdrop-blur-md">
                <p className="text-[10px] font-black uppercase tracking-widest text-minimal-accent mb-2">Network Status</p>
                <div className="flex items-center gap-2 mb-1">
                   <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
                   <span className="text-[10px] font-bold app-text">Nodes Active</span>
                </div>
                <p className="text-[9px] app-text-secondary leading-tight mt-2 italic">Tap any node to visualize the automatic routing paths.</p>
             </div>
          </div>
          ...
          <motion.div 
            ...
            style={{ y: useTransform(smoothProgress, [0, 0.3], [0, isMobile ? 0 : 100]) }}
      ...
      <section className="py-12 border-t border-b app-border bg-minimal-bg/30">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row items-center justify-between gap-8">
            <p className="text-sm font-black uppercase tracking-widest app-text-bold">Works worldwide. No configuration required.</p>
            <div className="flex flex-wrap justify-center gap-6">
              {[
                { icon: <Zap className="w-4 h-4"/>, label: "One-tap connection" },
                { icon: <Activity className="w-4 h-4"/>, label: "Instant activation" },
                { icon: <Globe className="w-4 h-4"/>, label: "Works on restricted networks" }
              ].map((badge, i) => (
                <div key={i} className="flex items-center gap-2 px-4 py-2 app-card border app-border rounded-full">
                  <div className="text-minimal-accent">{badge.icon}</div>
                  <span className="text-[9px] font-black uppercase tracking-wider app-text-secondary">{badge.label}</span>
                </div>
              ))}
            </div>
          </div>
